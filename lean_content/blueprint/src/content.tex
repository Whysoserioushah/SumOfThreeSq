...
\begin{definition}
\label{def:Nat_is_represented_by}
...
\begin{lemma}
\label{lem:QuadMap_equiv_apply}
\uses{def:QuadMap_equiv, lem:mat_symm_preserved}
...
\uses{def:QuadMap_equiv, def:Nat_is_represented_by, lem:QuadMap_equiv_apply}
\lean{Nat.IsRepresentedBy.of_equiv}
\leanok
...
\begin{proof}
\leanok 
Easy follows from ~\ref{lem:QuadMap_equiv_apply}.
\end{proof}
...
\begin{definition}
\label{def:QuadMap_posdef}
\lean{QuadraticMap.PosDef}
\mathlibok
...
\end{lemma}
\begin{proof}
\uses{def:QuadMap_equiv, def:QuadMap_posdef}
\leanok
...
\begin{proof}
\uses{lem:binary-reduction, def:QuadMap_equiv, lem:QuadMap_equiv_represents}
\leanok
Let $F$ be a positive–definite binary quadratic form of discriminant
...
\begin{proof}
\uses{lem:binary_posdef, lem:QuadMap_posdef_equiv}
\leanok
...
\begin{proof}
\leanok
\uses{def:QuadMap_Tenary_G, lem:QuadMap_posdef_equiv, lem:mat_symm_preserved, 
  lem:ternary_G_apply, lem:QuadMap_Tenary_G_det}
...
\uses{lem:ternary_posdef_iff, constr:mkSL3_from_coprime_int, lem:QuadMap_posdef_equiv, 
  lem:mat_symm_preserved, lem:ternary_G_apply, lem:QuadMap_Tenary_G_det}
...
\begin{lemma}
\label{lem:ternary_det_eq_one}
\lean{QuadraticMap.Tenary.det_eq_one}
\leanok 
Every positive–definite ternary quadratic form of discriminant $1$
is equivalent to the form
\[x_1^2 + x_2^2 + x_3^2.
\]
\end{lemma}
...
\begin{proof}
\leanok
\uses{lem:ternary_lemma6, thm:binary_quadMap_det_eq_one, lem:QuadMap_equiv_represents}
TO BE ADDED.
\end{proof}
...
\lean{Nat.quadResidue_to_sum_threeSq}
\leanok
...
\begin{proof}
\leanok
\uses{lem:ternary_det_eq_one, lem:ternary_posdef_iff}
...
By Lemma~\ref{lem:ternary_posdef_iff}, $F_A$ is positive–definite, and by construction
\[
F_A(0,0,1) = n,
\]
so $F_A$ represents $n$.

By Theorem~\ref{lem:ternary_det_eq_one}, every positive–definite ternary quadratic form of
...
\begin{lemma}
\label{lem:n2mod4}
\lean{Nat.mod_four_eq_two_to_sum_threeSq}
\leanok
...
\begin{lemma}
\label{lem:n135mod8}
\lean{Nat.sum_threeSq_of_mod_eight_eq_one, Nat.sum_threeSq_of_mod_eight_eq_three, 
  Nat.sum_threeSq_of_mod_eight_eq_five}
\leanok
...
\begin{theorem}[Gauss]
\label{thm:Gauss-three-squares}
\lean{Nat.sum_threeSq_iff}
\leanok
